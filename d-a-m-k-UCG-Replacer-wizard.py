...
# -*- coding: utf-8 -*-
__doc__ = """
Replaces specified glyphs from Font B in Font A, scales them by a percentage, resets kerning, and transfers spacing values and kerning groups from the original glyphs of Font A. Generates a report file on the desktop with changed letters.
"""
...
class DAMKReplacerWizard(object):
    ...
    def showUI(self):
        ...
        alert.setInformativeText_("Select (1) the target font and (2) the source font from the opened fonts. The source font will replace the glyphs in the target font based on a mapping file or text. In the box, select the scale factor.")
        alert.addButtonWithTitle_("Run")
        alert.addButtonWithTitle_("Cancel")

        font_a_popup = NSPopUpButton.alloc().initWithFrame_(((0, 90), (200, 25)))
        font_b_popup = NSPopUpButton.alloc().initWithFrame_(((0, 60), (200, 25)))
        ...
        scale_input = NSTextField.alloc().initWithFrame_(((0, 30), (100, 24)))
        scale_input.setStringValue_("100.0")

        # Mapping input
        mapping_input = NSTextField.alloc().initWithFrame_(((0, 0), (200, 24)))
        mapping_input.setPlaceholderString_("Paste mapping text here")

        custom_view = NSView.alloc().initWithFrame_(((0, 0), (200, 120)))
        custom_view.addSubview_(font_a_popup)
        custom_view.addSubview_(font_b_popup)
        custom_view.addSubview_(scale_input)
        custom_view.addSubview_(mapping_input)
        ...
        if response == 1000:  # NSAlertFirstButtonReturn
            ...
            self.parseMapping(mapping_input.stringValue())
            self.run()

    def parseMapping(self, mapping_text):
        """Parses the pasted mapping text to create a dictionary of replacements in 'source - target' format."""
        lines = mapping_text.strip().splitlines()
        self.mapping = {}
        for line in lines:
            parts = line.split("-")
            if len(parts) == 2:
                source_glyph = parts[0].strip()
                target_glyph = parts[1].strip()
                self.mapping[source_glyph] = target_glyph
    ...
    def run(self):
        ...
        try:
            ...
            # Iterate over glyphs based on provided mapping
            for source_name, target_name in self.mapping.items():
                glyph_a = self.font_a.glyphs[target_name]
                glyph_b = self.font_b.glyphs[source_name]
                
                if glyph_a and glyph_b:
                    self.replaceAndScaleGlyph(glyph_a, glyph_b, scale_factor)
                else:
                    self.unchanged_letters.append(target_name)
